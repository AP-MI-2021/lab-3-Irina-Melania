...
def test_get_longest_same_div_count():
    ...
    assert get_longest_same_div_count([2, 3, 4, 5]) == [3, 4]
...
def solve2(lst):
    ...
    def get_longest_product_is_odd(lst: list[int]):
        ...
        for j in range (i, lg):
            ...
            if produs % 2 == 1:
                dr = j
                if dr - st + 1 > secvmax: # in cazul in care lungimea secventei curente este mai mare decat secv maxima de pana acum
                    secvmax = dr - st + 1
                    start = st
                    end = dr
